...
**cognis-sources** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cognis-sources` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cognis-sources ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cognis-sources ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cognis-sources` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cognis-sources") for rec in records]   # records = your JSON output
