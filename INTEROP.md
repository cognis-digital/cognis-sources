...
How **cognis-sources** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `cognis-sources` lives in the **Dataset / catalog** cluster.
...
| from | relation | to |
|---|---|---|
| `cognis-sources` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis), [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint) |
| `cognis-sources` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `cognis-sources` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `cognis-sources` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the dataset / catalog cluster.** `cognis-sources` output feeds the next tool:
```bash
cognis-sources ... --format json > out.json          # this tool's findings (see `cognis-sources --help`)
...
cognis-sources ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
cognis-sources ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
cognis-sources ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `cognis-sources`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `cognis-sources` slots
